...
export default function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      setLoadingProduct(true)
      try {
        const response = await api.get<{ listing: any }>(`/api/listings/${params.id}`) // Use 'any' temporarily to handle flexible backend response
        const fetchedListing = response.listing

        if (fetchedListing) {
          // Prioritize 'id', but fallback to '_id' if backend uses MongoDB-style IDs
          const productId = fetchedListing.id || fetchedListing._id

          console.log("DEBUG: Raw productId from API response:", productId) // New log for raw ID

          if (productId) {
            setProduct({
              ...fetchedListing,
              id: String(productId), // Ensure ID is always a string
              reviews: fetchedListing.reviews || [], // Ensure reviews is an array, even if empty
            })
            console.log("DEBUG: Product state updated with ID:", String(productId)) // Updated log
          } else {
            console.error(
              "Failed to fetch product details: Product ID (id or _id) is missing in API response for listing:",
              fetchedListing,
            ) // More detailed error
            toast({
              title: "Error",
              description: "Product ID missing from API response. Please check backend data.",
              variant: "destructive",
            })
            setProduct(null)
          }
        } else {
          console.error("Failed to fetch product details: Listing object is null or undefined.", response)
          toast({
            title: "Error",
            description: "Failed to load product details. Listing not found.",
            variant: "destructive",
          })
          setProduct(null)
        }
  ...
  const handleSubmitReview = async () => {
    ...
    if (reviewComment.trim().length < 1) {
      ...
      toast({
        ...
        description: "Please provide a comment (at least 1 character).", // Updated message
        variant: "destructive",
      })
      return
    }
    ...
    if (!product || !product.id) {
      // Added explicit check for product.id
      console.error("ERROR: Product or Product ID is missing when submitting review.", { product }) // Added detailed error log
      toast({
        title: "Product Error",
        description: "Product information is not available for review submission. Please refresh the page.",
        variant: "destructive",
      })
      setIsSubmittingReview(false) // Ensure loading state is reset
      return
    }

    console.log("DEBUG: Attempting to submit review for product ID:", product.id) // Confirm ID before fetch
    ...
    try {
      ...
      const reviewData = {
        ...
        userId: user.uid,
        userName: (profile?.display_name || user.displayName || user.email || "Anonymous") as string,
        userEmail: user.email || undefined,
      }

      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/listings/${product.id}/reviews`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
      ...
      if (!res.ok) {
        ...
        try {
          ...
          console.error("Error parsing review error response:", e)
        }
        throw new Error(errorMessage)
      }

      const newReview = await res.json() // Assuming the API returns the newly created review
      ...
      setProduct((prevProduct) => {
        ...
        return {
          ...
          reviews: [
            ...prevProduct.reviews,
            {
              id: newReview.id || String(prevProduct.reviews.length + 1), // Ensure ID is string
              name: reviewData.userName || "Anonymous", // Ensure name is string, add fallback
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : "Failed to submit your review. Please try again.",
        variant: "destructive",
      })
    } finally {
  ...
  const averageRating =
    product.reviews.length > 0
      ...
      : product.artisan.rating.toFixed(1) // Fallback to artisan rating if no product reviews
        ...
        <div className="mt-16 space-y-12">
          ...
          <Card className="border-orange-200">
            <CardHeader>
              <CardTitle>Customer Reviews</CardTitle>
              <CardDescription>
                {product.reviews.length} reviews with an average rating of {averageRating} stars
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                {product.reviews.map((review, index) => (
                  <div
                    key={`${review.id || index}-${review.name}`}
                  ...
                  <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                      ...
                      <Button
                        ...
                        disabled={isSubmittingReview || reviewRating === 0 || reviewComment.trim().length < 1}
                      >
                        {isSubmittingReview ? (
                          <>
                            <Loader2 className="w-4 h-4 mr-2 animate-spin" />
